...
def learn(env, test_env, policy_func, *,
    ...
    # FIXME： no cofficient between different loss
    ...
    for update in range(1, max_iters + 1):
        ...
        if schedule == 'constant':
            cur_lrmult = adv_lrmult = 1.0
            print(schedule)

        elif schedule == 'linear':
            cur_lrmult = adv_lrmult = 1.0 - (update - 1.0) / max_iters
            print(schedule)

        elif schedule == 'asym':
            # cur_lrmult = 1.0 - (update - 1.0) / max_iters
            # adv_lrmult = cur_lrmult / 10
            #
            scale += 1
            adv_lrmult = 1.0 - (update - 1.0) / max_iters
            cur_lrmult = adv_lrmult / (200 - update * 0.33)
            print([schedule, 200 - update * 0.33])
        ...
        # TODO: training the protagnist agent
        ...
        # TODO：Training the adversary agent
        ...
        for _ in range(optim_epochs):
            ...
            for batch in d.iterate_once(optim_batchsize):
                *newlosses, g = adv_lossandgrad(batch["ob"], batch["ac"], batch["atarg"], batch["vtarg"], adv_lrmult)
                adv_adam.update(g, optim_stepsize * adv_lrmult)
                adv_losses.append(newlosses)

        adv_losses = []
        for batch in d.iterate_once(optim_batchsize):
            newlosses = adv_compute_losses(batch["ob"], batch["ac"], batch["atarg"], batch["vtarg"], adv_lrmult)
            adv_losses.append(newlosses)
